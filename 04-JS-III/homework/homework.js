...
function agregarNumeros(numeros) {
  ...
  for(i=0; i < numeros.length; i++){
    acomuladoes = acomuladoes + numeros[i];
  }
  return acomuladoes
}
...
  // Si no se pasan argumentos devuelve 0. 
  ...
  if(arguments.length === 0){
    ...
    for(i = 0; i < arguments.length; i++){
      acomulado = acomulado * arguments[i];  
    }
    return acomulado
  }
}
...
function cuentoElementos(arreglo){
  ...
  for(i = 0; i < arreglo.length; i++){
    if(arreglo[i] > 18){
      arrayAcomulado.push(arreglo[i]);
    }
  }
  return arrayAcomulado.length;
}
...
function mayorACien(array) {
  ...
  //Recorrer el array y guardar en un nuevo array sólo los valores mayores a 100 (no incluye el 100). 
